...
#######################################################################
### plots with full lines ###
cbbPalette <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
#flying = 8; resting = 4
### Read in data for BB
BB_Full = read.csv('Flying_Bumblebee_LookupTable_Full.csv',header=F)
colnames(BB_Full) = c("resting","shivering","flying")
LethalTemp_BB = 45  #lethal thorax/air temp in C
CoolingTemp_BB = 42  #thorax temp where cooling begins
FlyingTemp_BB = 30  #thorax temp where flight can begin
...
### Read in data for HB
HB_Full = read.csv('Flying_Honeybee_LookupTable_Full.csv',header=F)
colnames(HB_Full) = c("resting","shivering","flying")
LethalTemp_HB = 52  #lethal thorax/air temp in C
CoolingTemp_HB = 47.9  #thorax temp where cooling begins
FlyingTemp_HB = 35  #thorax temp where flight can begin

#plot for BB
plot(BB_Full[,1],lty=1,type="l",xlim=c(0,50),ylim=c(25,52),col=cbbPalette[4],
     main = "Bumblebee",ylab="Equilibrium thorax temperature (C)",xlab = "Air temperature (C)",
     cex.axis=1.5,cex.lab=1.5,lwd=2)
points(BB_Full[,3],lty=1,type="l",col=cbbPalette[8],lwd=2)
points(BB_Full[,7],lty=2,type="l",col=cbbPalette[4],lwd=2)
points(BB_Full[,9],lty=2,type="l",col=cbbPalette[8],lwd=2)
polygon(x=c(-5,55,55,-5),y=c(FlyingTemp_BB,FlyingTemp_BB,LethalTemp_BB,LethalTemp_BB)
        ,col=rgb(86/255,180/255,233/255,alpha=0.25),border=NA)


### plot for HB
plot(HB_Full[,1],lty=1,type="l",xlim=c(0,50),ylim=c(25,52),col=cbbPalette[4],
     main = "Honeybee",ylab="Equilibrium thorax temperature (C)",xlab = "Air temperature (C)",
     cex.axis=1.5,cex.lab=1.5,lwd=2)
points(HB_Full[,3],lty=1,type="l",col=cbbPalette[8],lwd=2)
points(HB_Full[,7],lty=2,type="l",col=cbbPalette[4],lwd=2)
points(HB_Full[,9],lty=2,type="l",col=cbbPalette[8],lwd=2)
polygon(x=c(-5,55,55,-5),y=c(FlyingTemp_HB,FlyingTemp_HB,LethalTemp_HB,LethalTemp_HB)
        ,col=rgb(213/255,94/255,0/255,alpha=0.3),border=NA)
...
# where the bee reaches flight possible temp:
HB_nocooling_fly_index = min(which(HB_Full[,3]>=FlyingTemp_HB)) #flying, no cooling
HB_cooling_fly_index = min(which(HB_Full[,9]>=FlyingTemp_HB)) #flying, cooling
HB_nocooling_fly_airtemp = HB_nocooling_fly_index-1
HB_cooling_fly_airtemp = HB_cooling_fly_index-1

BB_nocooling_fly_index = min(which(BB_Full[,3]>=FlyingTemp_BB)) #flying, no cooling
BB_cooling_fly_index = min(which(BB_Full[,9]>=FlyingTemp_BB)) #flying, cooling
BB_nocooling_fly_airtemp = BB_nocooling_fly_index-1
BB_cooling_fly_airtemp = BB_cooling_fly_index-1


# where the bee reaches thermal maximum:
HB_nocooling_dead_index = min(which(HB_Full[,3]>=LethalTemp_HB)) #flying, no cooling
HB_cooling_dead_index = min(which(HB_Full[,9]>=LethalTemp_HB)) #flying, cooling
HB_nocooling_dead_airtemp = HB_nocooling_dead_index-1
HB_cooling_dead_airtemp = HB_cooling_dead_index-1

BB_nocooling_dead_index = min(which(BB_Full[,3]>=LethalTemp_BB)) #flying, no cooling
BB_cooling_dead_index = min(which(BB_Full[,9]>=LethalTemp_BB)) #flying, cooling
BB_nocooling_dead_airtemp = BB_nocooling_dead_index-1
BB_cooling_dead_airtemp = BB_cooling_dead_index-1
